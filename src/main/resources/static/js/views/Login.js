    ...
    $('#app').click(function (e) {
        if (e.target.id === 'forgotPasswordLink') {
            let forgotPasswordForm =
                //language=html
                ` <label for="forgot-my-password-email" class="px-[10px] my-auto text-center text-3xl">A reset password link will be sent to your
	            email</label>
            ...
            <button id="recovery-email" type="button" class="w-full p-2 mx-2 my-2 rounded-md shadow-xl bg-callToAction font-medium">
               ...
               <p id="recovery-email-sent" class="hidden bg-slate-200 border-b-2 border-callToAction outline-0 placeholder-primary font-medium w-full mx-1 my-3 p-1 text-2xl">If the email is associated with a registered user a link to reset password will be sent </p>
            `
        ...
        if(e.target.id === 'recovery-email'){
            ...
            fetchData({server: `/api/users/send?userEmail=${userEmail}`}, request)
                .then(response => {
                    ...
                    alert("Link will be sent to email on file--Be sure to check spam for Team Clear to Close")
                    createView("/");
                })
        }
    })
