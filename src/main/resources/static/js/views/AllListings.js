...
export default function AllListings(props) {
    ...
    return props.allListings.map(listing => `
        ...
        <div class="w-11/12 flex flex-wrap justify-evenly border-2 border-callToAction bg-secondary m-2"
             data-id="${listing.id}" id="listings">
...
function codeAddress(address) {
        for (address = 0; address < locations.length; address++) {
            const marker = new google.maps.Marker({
                position: new google.maps.LatLng(locations[count][i], locations[count][i]),
                map: this.map,
