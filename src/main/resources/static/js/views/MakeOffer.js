...
export default function MakeOffer(props) {
    ...
    console.log(props);
    return `
		<div id="makeOfferParentDiv"
		     class="content-height bg-slate-200 opacity-95 flex flex-col items-center justify-center">
			<div class="w-3/4 md:w-1/2">
				<div class="flex flex-col items-center text-left justify-center my-3">
					<form class="flex flex-col items-center justify-center bg-white border-2 border-callToAction shadow-xl rounded-md w-full px-2 py-2 m-1">
						...
						<button id="postOfferBtn" type="button"
...
function submitOffer() {
    ...
    $('#postOfferBtn').on('click', function (e) {
        ...
        const offerData = {
            ...
            listingId: $('#postOfferBtn').data('id'),
            offerStatus: `ACTIVE`
        ...
        fetchData({server: `/api/offers`}, postRequest).then(response => {
            createView({
                offers: {
                    offers: `/api/offers/findOffers/${$('#postOfferBtn').data('id')}`,
                    listing: `/api/listings/${$('#postOfferBtn').data('id')}`,
